...
class InvoiceResource extends Resource implements HasShieldPermissions
{
    ...
    public static function getNavigationBadge(): ?string
    {
        ...
        // $activeInvoice = Invoice::where('status', 'active')->count();
        // return " active invoices: $activeInvoice";
    }

    // public static function getNavigationBadgeTooltip(): ?string // when you hover the mouse on the badge
    // {
    //     return __('active invoices');
    // }
        ...
        return $form
            ->schema([
                ...
                Forms\Components\Section::make(__('Invoice Items'))
                    ->schema([
                        Forms\Components\Repeater::make(__('items'))
                            ->relationship('items')
                            ->schema([
                ...
                Forms\Components\Section::make(__('Additional Information'))
                    ->schema([
                        Forms\Components\TextInput::make('delivery_fee')
                            ...
                            ->prefix('€')
                            ->default(0)
                            ->extraAttributes(['wire:key' => 'delivery_fee_field'])  // Add a unique wire:key
                            ->columnSpan(1),
    ...
    {
        return $table
            // ->query(Invoice::query()->where('status', '!=', 'archived'))
            ->columns([
